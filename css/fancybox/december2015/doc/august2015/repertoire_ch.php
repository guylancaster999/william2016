<?php 
require "funcz/functionz.php";
print head("ch","剧目");
?>
<body>
    <div id="wrapper">
       <div id="sidebar-wrapper">
            <?php    print  menu("ch","剧目");	?>
         </div>
          <div id="page-content-wrapper">
            <div class="container-fluid"> 
			   <?php topRight("repertoire_e.php","repertoire.php","repertoire_ch.php"); ?>
                <div class="row">
	                <div class="col-lg-12"> 
                    <h2>剧目</h2> 
    	            </div>
        	     </div>
            	 <div class="row">
                	 <div class="col-lg-12">         
       					<div class="alpha">        
			 <a href="#a">a</a> &nbsp;|&nbsp; 
<a href="#b">b</a> &nbsp;|&nbsp; 
<a href="#c">c</a> &nbsp;|&nbsp; 
<a href="#d">d</a> &nbsp;|&nbsp; 
e &nbsp;|&nbsp; 
<a href="#f">f</a> &nbsp;|&nbsp; 
<a href="#g">g</a> &nbsp;|&nbsp; 
<a href="#h">h</a> &nbsp;|&nbsp; 
i &nbsp;|&nbsp; 
<a href="#j>j</a> &nbsp;|&nbsp; 
k &nbsp;|&nbsp; 
<a href="#l">l</a> &nbsp;|&nbsp; 
<a href="#m">m</a> &nbsp;|&nbsp; 
n &nbsp;|&nbsp; 
o &nbsp;|&nbsp; 
<a href="#p">p</a> &nbsp;|&nbsp; 
q &nbsp;|&nbsp; 
<a href="#r">r</a> &nbsp;|&nbsp; 
<a href="#s">s</a> &nbsp;|&nbsp; 
<a href="#t">t</a> &nbsp;|&nbsp; 
u &nbsp;|&nbsp; v &nbsp;|&nbsp; 
<a href="#w">w</a> &nbsp;|&nbsp; x &nbsp;|&nbsp; y &nbsp;|&nbsp; 
<a href="#z">z</a></div>
 		<div class="row">
                 <div class="col-sm-9">
                    <a href="http://en.wikipedia.org/wiki/Isaac_Alb%C3%A9niz"><h3>I.Albeniz</h3></a>
				<ul>
				<li>Pieces from Iberia (ab 2001)</li>
				<li>Tango, arranged by von  L. Godowski</li>
				</ul>
	      	</div>
                 <div class="col-sm-3">
                  <?php photorightnolink("Albeniz.jpg","I.Albeniz:%20Photo:Wikipedia");?> 
                   </div>
             </div>
             <?php blank_line();?> 
              
  <div class="row">
                 <div class="col-sm-9">
                <a id="b"></a> 
 				<a href="http://en.wikipedia.org/wiki/Johann_Sebastian_Bach"><h3>J.S.Bach</h3></A>
<ul>
<li>Partita Nr. 1 B flat major</li>
<li>Partita Nr. 6 E minor</li>
<li>Preludes and fugues from the Well-Tempered Clavier Book 1: C major, C minor, C# major, C sharp minor, D major, D minor, E-flat, E-flat minor, E major, F minor, G major, B-flat, B-flat minor.</li>
<li>Preludes and fugues from the Well-Tempered Clavier Book 2: D major, D minor, E-flat, F minor, F sharp minor, G major, A-flat major</li>
<li>Goldberg Variations BWV 988</li>
<li>Toccata  E minor BWV 914</li>
<li>Italian Concerto BWV 971</li>
</ul>
					</div>
                 <div class="col-sm-3">
                 <?php photorightnolink("Johann_Sebastian_Bach.jpg","Johann_Sebastian_Bach:%20Photo:Wikipedia");?>  
                </div>
             </div>

       <?php blank_line();?> 

    <div class="row">
                 <div class="col-sm-9">
              		<a href="http://en.wikipedia.org/wiki/Mily_Balakirev"><h3>M.I.Balakirev</h3></a>
					<ul>
						<li>Sonate B flat minor</li>
						<li>Islamey</li>
					</ul>
				</div>
                 <div class="col-sm-3">
                 <?php photorightnolink("Balakirev.jpg","Milij_Aleksejevic_Balakirev:%20Photo:Wikipedia");?>  
                </div>
              </div>
              
			  <div class="row">
                 <div class="col-sm-12">
              	   &nbsp;
                 </div>
               </div>
                <div class="row">
                 <div class="col-sm-9">
                         <a href="http://en.wikipedia.org/wiki/Ludwig_van_Beethoven"><h3>L.V.Beethoven</h3></a>
<ul>
  <li>Sonata in A Major Op. 2, No. 2</li>
  <li>Sonata in C major Op. 2, No. 3</li>
  <li>Sonata in E flat major Op. 7</li>
  <li>Sonata in C minor Op. 13 &quot;Pathetique&quot;</li>
  <li>Sonata in A flat major Op. 26</li>
  <li>Sonata in C sharp minor Op. 27, No. 2 &quot;Moonlight&quot;</li>
  <li>Sonata in D Major Op. 28</li>
  <li>Sonata in C major Op. 53 &quot;Waldstein&quot;</li>
  <li>Sonata in F minor Op. 57 &quot;Appassionata&quot;</li>
  <li>Sonata in F sharp major, Op. 78</li>
  <li>Sonata in A Major Op. 101</li>
  <li>Sonata in B Flat Major Op. 106 &quot;Hammerklavier&quot;</li>
  <li>Sonata in E major Op. 109</li>
  <li>Sonata in A flat major Op. 110</li>
  <li>Sonata in C minor Op.111</li>
     </ul>

                     </div>
                 <div class="col-sm-3">
 				     <?php photorightnolink("Beethoven.jpg","L.V.Beethoven:%20Photo:Wikipedia");?> 
                 </div>
 				</div>
       <?php blank_line();?> 
<div class="row">
                 <div class="col-sm-9">
              <A href="http://en.wikipedia.org/wiki/Johannes_Brahms"><h3>J.Brahms</h3></A>
<ul>
  <li>16 Waltzes Op. 39</li>
  <li>Paganini Variations book1</li>
  <li>2 Rhapsodies Op. 79</li>
  <li>Op. 118 No. 1, No. 2</li>
  <li>Op. 119 No. 1, No. 2</li></ul>
  </div>
  <div class="col-sm-3">
      <?php photorightnolink("JohannesBrahms.jpg","Johannes%20Brahms:%20Photo:Wikipedia")?>
  </div>
  </div>
<div class="row">
                 <div class="col-sm-12">
              	   &nbsp;
                 </div>
               </div>
<div class="row">
                 <div class="col-sm-9">
  <a id="c"></a>
<a href="http://en.wikipedia.org/wiki/Fr%C3%A9d%C3%A9ric_Chopin"><h3>F.Chopin</h3></a>
<ul>
  <li>The 4 Ballades</li>
  <li>The Preludes</li>
  <li>The 3 Sonatas</li>
  <li>The 4 Scherzi</li>
  <li>The etudes</li>
  <li>The Waltz</li>
  <li>selected mazurkas</li>
  <li>selected Nocturnes</li>
  <li>selected Polonaises (also Polonaise-Fantasie and Grande polonaise brillante Andante et Spinato)</li>
  <li>Fantasy</li>
  <li>Barcarolle</li>
  <li>Berceuse</li>
  <li> Tarantella  Op.43</li>
</ul>
  </div>
  <div class="col-sm-3">
      <?php photorightnolink("Chopin.png","F.Chopin:%20Photo:Wikipedia");?>
  </div>
  </div>
<div class="row">
                 <div class="col-sm-9">
<a id="d"></a>
<a href="http://en.wikipedia.org/wiki/Claude_Debussy"><h3>C.Debussy</h3></a>
<ul>
<li>Children´s Corner</li>
<li>Preludes: Des pas sur la neige</li>
<li>La fille aux cheveux de lin</li>
<li>La cathédrale engloutie</li>
<li>General Lavine - eccentric</li>
<li>Deux Arabesques</li>
<li>Claire de lune</li>
<li>Reflets dans l'eau ( ab 2001 )</li>
</ul>
</div>
                 <div class="col-sm-3">
<?php photorightnolink("Claude_Debussy.jpg","Claude_Debussy:%20Photo:Wikipedia");?>
</div>
         </div>

<div class="row">
    <div class="col-sm-9">
<a id="f"></a>
<A href="http://en.wikipedia.org/wiki/John_Field_(composer)"><h3>J. Field</h3></A>
<ul>
<li>Nocturne in A major</li>
</ul>
</div>
    <div class="col-sm-3">
   <?php photorightnolink("John_field.jpg","J.Field:%20Photo:Wikipedia");?>
    </div>
</div>

<div class="row">
                 <div class="col-sm-12">
              	   &nbsp;
                 </div>
               </div>

<div class="row">
    <div class="col-sm-9">
			<a href="http://en.wikipedia.org/wiki/C%C3%A9sar_Franck"><h3>C.Franck.</h3></a>
<ul>
<li>Variations Symphoniques (Piano und Orchestra)</li>
<li>Prelude, Chorale and Fugue</li>
</ul>
</div>
<div class="col-sm-3">
   <?php photorightnolink("Cesar_Franck.jpg","Cesar%20Franck:%20Photo:Wikipedia");?>
</div>
   </div>
<?php blank_line();?>   
   <div class="row">
   <div class="col-sm-9">
		<a id="g"></a>
        <A href="http://en.wikipedia.org/wiki/George_Gershwin"><h3>G.Gershwin</h3></A>
<ul>
<li><?php linkedPhoto("Rhapsodie-in-Blue","Gershwin2.JPG","Repertoire","repertoire_e.php","en");?></li>
</ul>
</div>
		<div class="col-sm-3">
			  <?php photorightnolink("George_Gershwin.jpg","George_Gershwin:%20Photo:Wikipedia");?>
        </div>
</div>
               <?php blank_line();?>   
               <div class="row">
                 <div class="col-sm-9">
              	   <A href="http://en.wikipedia.org/wiki/Mikhail_Glinka"><h3>M.Glinka</h3></A>
<ul>
  <li>"The Lark" arr. M.Balakirew</li>
  </ul>
</div>
<div class="col-sm-3">
			  <?php photorightnolink("Mikhail_Glinka.jpg","M.Glinka:%20Photo:Wikipedia");?>
</div>
</div>
            <?php blank_line();?>   
<div class="row">
   <div class="col-sm-9">
<A href="http://en.wikipedia.org/wiki/Enrique_Granados"><h3>E.Granados </h3></A>
<ul>
  <li>The Maiden and the Nightingale ( Goyescas)</li>
        </ul>
        </div>
        <div class="col-sm-3">
           <?php photorightnolink("Granados.jpg","Enrique_Granados:%20Photo:Wikipedia");?>
        </div>
     </div>
      <?php blank_line();?>   
<div class="row">
   <div class="col-sm-9">
		<a href="http://en.wikipedia.org/wiki/Edvard_Grieg"><h3>E.Grieg</h3></a>
<ul>
  <li>Some works from &quot;Lyric Pieces&quot;</li>
  <li>Sonata in E minor Op.7</li>
  <li>Suite &quot;From Holberg's Time&quot; Op.40</li>
  </ul>
     </div>
   <div class="col-sm-3">
     <?php photorightnolink("Edvard_Grieg.jpg","E.Grieg:%20Photo:Wikipedia");?>
      </div>
     </div>
     <?php blank_line();?>   
     <div class="row">
       <div class="col-sm-9">
       <a id="h"></a>
           <a href="http://en.wikipedia.org/wiki/Joseph_Haydn"><h3>J.Haydn</h3></a>
<ul>
  <li>Variations in F minor</li>
  <li>Variations in E flat major</li>
</ul>
               </div>
               <div class="col-sm-3">
                   <?php photorightnolink("Joseph_Haydn.jpg","J.Haydn:%20Photo:Wikipedia");?>
               </div>
            </div>   

 <?php blank_line();?> 
     <div class="row">
       <div class="col-sm-9">
<a id="j"></a>&nbsp;
<a href="https://en.wikipedia.org/wiki/Leo%C5%A1_Jan%C3%A1%C4%8Dek"><h3>L.Janacek</h3></a>
<ul>
  <li >Auf verwachsenem Pfad</li>
  </ul>
</div>
<div class="col-sm-3">
		<?php  photorightnolink("Leos_Janacek.jpg","Janacek-Photo:Wikipedia");?>
</div>
</div>

		<?php blank_line();?> 
<div class="row">
  <div class="col-sm-9">
    <a href="http://en.wikipedia.org/wiki/Franz_Liszt"><h3>F.Liszt</h3></a>
 <ul>
  <li> Sonata in B minor</li>
<li>Mephisto Waltz</li>
<li>Hungarian Rhapsody Nr.2,4,6</li>
<li>Liebestraume &ndash; Three Notturnos</li>
<li>Valse oubli&eacute;e Nr.1 in Fis&ndash;Dur, Nr.2 in As&ndash;Dur</li>
<li>Consolation Nr.1 in E-Dur, Nr.2 in E-Dur, Nr.3 in Des&ndash;Dur, Nr.4 in Des&ndash;Dur, Nr.5 in E&ndash;Dur, Nr.6 in E&ndash;Dur</li>
<li>Chopin/Liszt Ein Madchens Wunsch</li>
<li>Etudes:La leggierezza,Un sospiro, La campanella, Feux Follets &ndash; Irrlichter (Etude d'execution transcendente Nr. 5) Zwei Konzert Etuden: 1. Waldesrauschen, 2. Gnomenreigen</li>
<li>aus Ann&eacute; de pelerinage
<ul>
<li>Premi&egrave;re Ann&eacute;e &ndash; Suisse :Au lac de Wallenstadt ,Au bord d'une source ,Vallee d'Obermann</li>
<li>Deuxi&egrave;me ann&eacute;e: Italie :104th Sonnetto del Petrarca</li>
<li>Troisi&egrave;m&eacute; &ndash; Italie :Les jeux d'eaux a la Villa d'Este</li>
 </li>
</ul>
</li>
</ul>	
	</div>
			<div class="col-sm-3">
				<?php  photorightnolink("Franz_Liszt.jpg","Franz.List:%20Photo:Wikipedia");?>
			</div>
		</div>

<?php blank_line();?> 

     <div class="row">
       <div class="col-sm-9">
<a id="l"></a>
<a href="http://en.wikipedia.org/wiki/Anatoly_Lyadov"><h3>A.Lyadov</h3></a>
<ul>
  <li>Variations on a Polish Folk Song</li>
  <li>Some Preludes</li>
  </ul>
</div>
<div class="col-sm-3">
		<?php  photorightnolink("Anatoly_Lyadov.jpg","Anatoly_Lyadov:%20Photo:Wikipedia");?>
</div>
</div>


		<?php blank_line();?> 
<div class="row">
  <div class="col-sm-9">
<a id="m"></a>
<a href="http://en.wikipedia.org/wiki/Felix_Mendelssohn"><h3>F.Mendellsohn</h3></a>
<ul>
  <li>Rondo Capriccioso Op.14</li>
  <li>Variations  serieuses Op.54</li>
  <li>Fantasia on 'The Last Rose' Op.15</li>
  <li>Songs without Words:
    <ul>
      <li>Op.19 Nr.1.2,4,6</li>
      <li>Op. 30 Nr.1,2,3,6</li>
      <li>Op. 38 Nr.2,4,6      </li>
      <li>Op. 53 Nr.2,4      </li>
      <li>Op. 62 Nr. 3,6      </li>
      <li>Öp.67 Nr. 2,4      </li>
      <li>Op. 102 Nr.3,5</li>
    </ul>
  </li>
</ul>
</div>
              <div class="col-sm-3">
                 <?php  photorightnolink("Mendelssohn.jpg","F.Mendelssohn:%20Photo:Wikipedia"); ?>
              </div>
           </div>
<?php blank_line();?> 
   <div class="row">
     <div class="col-sm-9">
        <a href="http://en.wikipedia.org/wiki/Ignaz_Moscheles"><h3>I.Moscheles</h3></a>
<ul>
<li>Etudes Op. 70 Nr. 5, Nr. 9</li></ul>
         </div>
         <div class="col-sm-3">
              <?php  photorightnolink("Ignaz_Moscheles.jpg","Ignaz%20Moscheles:%20Photo:Wikipedia"); ?>
         </div>
       </div>  
       <?php blank_line();?> 
   <div class="row">
     <div class="col-sm-9">
          <a href="http://en.wikipedia.org/wiki/Wolfgang_Amadeus_Mozart"><h3>W.A.Mozart</h3></a>
<ul>
  <li>Sonata in A minor, K. 310</li>
  <li>Sonata in C major K 330</li>
  <li>Sonata in A major, K 331</li>
  <li>Sonata in F major K 332</li>
  <li>Sonata in B flat major K 333</li>
  <li>Sonata in C Major K. 545</li>
  <li>Sonata in B flat major, K. 570</li>
  <li>Sonata in D major, K. 576</li>
  <li>Variations on &quot;Ah, vous dirai-je, Maman&quot; K. 265</li>
  <li>Fantasie in D minor K. 397</li>
</ul>
   </div>
   <div class="col-sm-3">
       <?php  photorightnolink("Mozart.jpg","w.a.Mozart:%20Photo:Wikipedia"); ?>
      </div>
   </div>
    <?php blank_line();?> 
    <div class="row">
      <div class="col-sm-9">
       <a href="http://en.wikipedia.org/wiki/Modest_Mussorgsky"><h3>M.Mussorgski</h3></a>
		<ul>
		<li>Pictures at an Exhibition</li>
		</ul>
	</div>
	 <div class="col-sm-3">
	       <?php  photorightnolink("Modest_Musorgskiy.jpg","M.Mussorgski:%20Photo:Wikipedia"); ?>
     </div>
   </div>
 <?php blank_line();?> 
    <div class="row">
      <div class="col-sm-9"> 
<a id="p"></a>
<a href="http://en.wikipedia.org/wiki/Sergei_Prokofiev"><h3>S.Prokofiev</h3></a>
<ul>
<li>Sonata No.3 in A minor Op. 28</li>
</ul>
</div>
<div class="col-sm-3">
           <?php  photorightnolink("Sergei_Prokofiev.jpg","S.Prokofiev"); ?>
      </div>
  </div>
<?php blank_line();?> 
 <div class="row">
      <div class="col-sm-9"> 
<a id="r"></a>
<a href="http://en.wikipedia.org/wiki/Sergei_Rachmaninoff"><h3>S.Rachmaninov</h3></a>
<ul>
  <li>Corelli Variations, Op. 42</li>
  <li>Etudes Tableaux Nos. 2, 5, 7, Op.33</li>
  <li>9 Etudes Tableaux Op. 39</li>
  <li>Polka de W.R</li>
  <li>10 Preludes Op. 23</li>
  <li>Preludes No. 1, 2, 5, 7, 10, 12 Op. 32</li>
  <li>Piano Concerto No. 3 in D minor, Op. 30</li>
<li>Piano Concerto No. 1</li>

  <li>Sonata in B flat minor Op. 36</li>
  </ul>
			</div>
		<div class="col-sm-3">
   	     <?php  photorightnolink("Sergei_Rachmaninoff.jpg", "Sergei_Rachmaninoff:%20Photo:Wikipedia"); ?>
     	
        </div>
</div>
<?php blank_line();?> 
 <div class="row">
      <div class="col-sm-9"> 
<a href="http://en.wikipedia.org/wiki/Maurice_Ravel"><h3>M.Ravel</h3></a>
<ul>
  <li>Valses nobles et sentimentales</li>
  <li>Sonatine</li>
  <li>Alborado del grazioso</li>
  <li>Jeux d'eau</li>
</ul>
			</div>
		<div class="col-sm-3">
   	        <?php  photorightnolink("Maurice_Ravel.jpg","Maurice_Ravel:%20Photo:Wikipedia"); ?>
     	</div>
</div>
<?php blank_line();?> 
 <div class="row">
      <div class="col-sm-9"> 
<a id="s"></a>
<a href="http://en.wikipedia.org/wiki/Franz_Schubert"><h3>F.Schubert</h3></a>
<ul>
  <li>Sonata in B major, Op. Posth. 147</li>
  <li>Sonata in A Major Op. Posth. 120</li>
  <li>Sonata in A minor Op. Posth. 143</li>
  <li>Sonata in A minor Op. 42</li>
  <li>Sonata in B flat major, D. 960</li>
  <li>Wanderer Fantasy in C major Op. 15</li>
  <li>Impromptu in E flat major Op. 90 No. 2</li>
  <li>Impromptu in G flat major, Op. 90 No. 3</li>
  <li>Six Moments Musicaux Op. 94</li>
  <li>Impromptu in A flat major, Op. Posth. 142 No. 2</li>
  <li>Impromptu in B flat major Op. Posth. 142 No. 3</li>
</ul>
</div>
	<div class="col-sm-3">
            <?php  photorightnolink("Franz_Schubert.jpg","Franz_Schubert:%20Photo:Wikipedia"); ?>
        </div>
</div>
<?php blank_line();?> 

	<div class="row">
		<div class="col-sm-9">
			<a href="http://en.wikipedia.org/wiki/Robert_Schumann"><h3>R.Schumann</h3></a>
<ul>
  <li>Abegg Variations Op.1</li>
  <li>Papillons Op.2</li>
  <li>Op.7 Tocata</li>
  <li>Carnival Op.9</li>
  <li>Etudes Op.13 Symphoniques</li>
  <li>Op.15 Kinderszenen</li>
  <li>Fantasy in C major Op.17</li>
</ul>
</div>
	<div class="col-sm-3">
	         <?php  photorightnolink("Schumann.jpg","Robert%20Schumann:%20Photo:Wikipedia"); ?>
      </div>
</div>
<?php blank_line();?> 

	<div class="row">
		<div class="col-sm-9">
		<a href="http://en.wikipedia.org/wiki/Alexander_Scriabin"><h3>A.Scriabin [2015 is the centenery of his death]</h3></a>
	<ul>
  <li>Deux Poèmes</li>
  <li>24 Preludes Op. 11</li>
  <li>Etudes Op. 2 No. 2, Op. 8, No. 5, 11, 12,  Op. 42, Op. 65 No. 1</li>
  <li>Piano Concerto in F sharp minor, Op. 20</li>
  <li>Prelude and Nocturne for the Left Hand</li>
  <li>Sonata No. 4 in F# Major, No 5, No 6, No 10</li>
  <li>Vers la flamme Op. 72</li>
</ul>
</div>
	<div class="col-sm-3">
      <?php  photorightnolink("Skrjabin_Alexander.jpg","Skrjabin_Alexander:%20Photo:Wikipedia");?>
	</div>
</div>
<?php blank_line();?> 

	<div class="row">
		<div class="col-sm-9">
<a href="http://en.wikipedia.org/wiki/Richard_Strauss"><h3>R.Strauss</h3></a>
<ul>
<li>Arrangement by Schulz-Evler,An die schöne blaue Donau</li>
<li>Arrangement by Godowski,Fledermaus</li></ul>
</div>
	<div class="col-sm-3">
      <?php  photorightnolink("Richard_Strauss.jpg","Richard_Strauss:%20Photo:Wikipedia");?>
	</div>
</div>
<?php blank_line();?> 

	<div class="row">
		<div class="col-sm-9">
			<a href="http://en.wikipedia.org/wiki/Karol_Szymanowski"><h3>K.Szymanowski</h3></a>
<ul>
<li>9 Preludes Op. 1</li></ul>
</div>
		<div class="col-sm-3">
      <?php  photorightnolink("Karol_Szymanowski.jpg","Karol_Szymanowski:%20Photo:Wikipedia");?>
</div></div>
<?php blank_line();?> 
	<div class="row">
		<div class="col-sm-9">
<a id="t"></a>
<a href="http://en.wikipedia.org/wiki/Pyotr_Ilyich_Tchaikovsky"><h3>P.I.Tchaikovsky</h3></a>
<ul>
  <li>The Seasons, Op. 37 a</li>
  <li>Doumka Op. 59</li>
</ul>

</div>
		<div class="col-sm-3">
      <?php  photorightnolink("Pjotr_I._Tschaikowski.jpg","Pjotr_I._Tschaikowski:%20Photo:Wikipedia");?>
</div></div>
<?php blank_line();?> 
	<div class="row">
		<div class="col-sm-9">
 			<a id="w"></a>
				<a href="https://en.wikipedia.org/wiki/Carl_Maria_von_Weber"><h3>C.M. von Weber</h3></a>
<ul>
<li>Sonate Nr. 2 As-Dur</li></ul>
 
</div>
<div class="col-sm-3">
	<?php  photorightnolink("Carl-Maria-Von-Weber.jpg","C.M. Von Weber");?>
</div>
</div>
<?php blank_line();?> 
<div class="row">
		<div class="col-sm-9">
 	<a id="z"></a>
<a href="http://en.wikipedia.org/wiki/Alexander_von_Zemlinsky"><h3>A.Zemlinski</h3></a>
<ul>
<li>Rural Dances Op. 1 (from 2001)</li>
</ul>
</div>
<div class="col-sm-3">
	<?php  photorightnolink("Zemlinsky.jpg","Alexander_von_Zemlinsky:%20Photo:Wikipedia");?>
</div>
</div>
<div class="row">
		<div class="col-sm-12">
 <?php 	print foot();
				?>
                    </div>
                </div>
            </div>
        </div>        <!-- /#page-content-wrapper -->
    </div>    <!-- /#wrapper -->
 <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
 <script src="http://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js"></script>
</body>
</html>
